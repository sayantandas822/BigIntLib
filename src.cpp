...
BigInteger::BigInteger(string s) { // "string" constructor
    if(isdigit(s[0])){ // if not signed , isdigit is inbuilt C++ function to check
...
const string& BigInteger::getNumber() { // retrieves the number for user reference
    return number;
}
...
const bool& BigInteger::getSign() {// retrieves the sign for user reference
    return sign;
}

BigInteger BigInteger::absolute() {//return type is BigInteger
